...
import { Plus, X, ChevronDown, ChevronUp, Timer, Flag, TrendingUp, RefreshCw, LayoutDashboard, ClipboardList, Activity, Award, Moon, Sun } from "lucide-react";
...
export default function App() {
  ...
  // Choose which colors to use based on current mode
  ...
  // ---------- Helper functions (inside App, so they can use C) ----------
  ...
  // ---------- Component functions (inside App) ----------
  const SplitLadder = ({ run }) => {
  ...
  const CountdownLane = ({ title, dateLabel, days, color, sub }) => (
  ...
  const BestEffortsTable = ({ bestEfforts }) => {
    const rows = [
  ...
  const AddRunModal = ({ onClose, onSave }) => {
  ...
  const DashboardTab = ({ allRuns, onShowModal }) => {
  ...
  const StatsTotalsRow = ({ label, totals }) => {
  ...
  const StatistiekenTab = ({ connected }) => {
